...
Stacks.prototype.add = function (method, path, options, middlewares) {
  ...
  if (!http.METHODS.includes(upperMethod)) {
    throw new Error('Method ' + upperMethod + ' not implemented.');
  }

  this.registered[upperMethod].push({regex: regex, keys: keys, middlewares: middlewares});
};
...
Stacks.prototype.matchRequest = function (method, url) {
  ...
  for (var i = 0, len = stacks.length; i < len; i++) {
    var stack = stacks[i];
    var paramList = stacks.regex.test(url);

    if (paramList) {
      return {params: makeParams(stack.keys, paramList), middlewares: stack.middlewares};
    }
  }
};
